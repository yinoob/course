...
public class OssController {
    ...
    public ResponseDto fileUpload( FileDto fileDto) throws Exception {
        LOG.info("上传文件开始");
...
//        }
    ...
    public ResponseDto fileUpload(@RequestParam MultipartFile file, String used) throws Exception {
        LOG.info("上传文件开始");
        FileUseEnum useEnum = FileUseEnum.getByCode(used);
        String keyId = UuidUtil.getShortUuid();
        String fileName = file.getOriginalFilename();
        String suffix = fileName.substring(fileName.lastIndexOf(".") + 1).toLowerCase();
        String dir = useEnum.name().toLowerCase();
        String path = dir + "/" + keyId + "." + suffix;
