...
import cn.wyslkl.server.domain.Sms;
import cn.wyslkl.server.domain.SmsExample;
import cn.wyslkl.server.dto.SmsDto;
import cn.wyslkl.server.dto.PageDto;
import cn.wyslkl.server.enums.SmsStatusEnum;
import cn.wyslkl.server.exception.BusinessException;
import cn.wyslkl.server.exception.BusinessExceptionCode;
import cn.wyslkl.server.mapper.SmsMapper;
...
public class SmsService {
    ...
    public void validCode(SmsDto smsDto) {
        ...
        SmsExample.Criteria criteria = example.createCriteria();
        // 查找5分钟内同手机号同操作发送记录
        criteria.andMobileEqualTo(smsDto.getMobile()).andUsedEqualTo(smsDto.getUsed()).andAtGreaterThan(new Date(new Date().getTime() - 1 * 60 * 1000));
        List<Sms> smsList = smsMapper.selectByExample(example);
