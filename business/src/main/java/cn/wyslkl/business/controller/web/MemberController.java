...
import cn.wyslkl.server.enums.SmsUseEnum;
import cn.wyslkl.server.exception.BusinessException;
...
public class MemberController {
    ...
    */
    @PostMapping("/login")
    public ResponseDto login( MemberDto memberDto, HttpServletRequest request) {
        LOG.info("用户登录开始");
        memberDto.setPassword(DigestUtils.md5DigestAsHex(memberDto.getPassword().getBytes()));
        ResponseDto responseDto = new ResponseDto();
        ...
        */
        ...
        request.getSession().setAttribute(Constants.LOGIN_MEMBER,loginMemberDto);
...
*/
    ...
    */
    @PostMapping(value = "/is-mobile-exist")///{mobile}
    public ResponseDto isMobileExist( String mobile) throws BusinessException {//@PathVariable(value = "mobile")
    ...
    public ResponseDto resetPassword( MemberDto memberDto) throws BusinessException {
        LOG.info("会员密码重置开始:");
        memberDto.setPassword(DigestUtils.md5DigestAsHex(memberDto.getPassword().getBytes()));
        ...
        smsDto.setMobile(memberDto.getMobile());
        smsDto.setCode(memberDto.getSmsCode());
        smsDto.setUsed(SmsUseEnum.FORGET.getCode());
        smsService.validCode(smsDto);
        LOG.info("短信验证码校验通过");
