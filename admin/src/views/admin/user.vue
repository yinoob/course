  ...
  export default {
    ...
    methods: {
      ...
      list(page) {
        let _this = this;
        //Loading.show();
        axios.post(process.env.VUE_APP_SERVER + '/system/admin/user/list', qs.stringify({
          page: page,
      ...
      del(id) {
        ...
        Confirm.show("删除用户表后不可恢复，确认删除？", function () {
          //Loading.show();
          axios.post(process.env.VUE_APP_SERVER + '/system/admin/user/delete',qs.stringify({id: id})).then((response)=>{
            //Loading.hide();
            let resp = response.data;
            if (resp.success) {
              _this.list(1);
      ...
      savePassword() {
        ...
        //Loading.show();
        _this.user.password = hex_md5(_this.user.password + KEY);
        axios.post(process.env.VUE_APP_SERVER + '/system/admin/user/save-password', qs.stringify(_this.user)).then((response)=>{
          //Loading.hide();
          let resp = response.data;
          if (resp.success) {
