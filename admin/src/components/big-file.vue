    ...
    methods: {
      uploadFile () {
        ...
        */
        ...
        let keyId = hex_md5(file.name + file.size + file.type);
        let keyId10 = parseInt(keyId, 16);
        let keyId62 = Tool._10to62(keyId10);
        console.log(keyId, keyId10, keyId62);
      ...
      */
      ...
      check (param) {
        let _this = this;
        _this.$ajax.get(process.env.VUE_APP_SERVER + '/file/admin/file/check/' +qs.stringify({ keyId:param.keyId})).then((response)=>{
          let resp = response.data;
          if (resp.success) {
